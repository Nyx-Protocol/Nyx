...
fn test_malformed_stream_frame_parsing() {
    ...
    assert!(
        result.is_err(),
        "STREAM frame with missing data should fail"
    );
    ...
    assert!(
        result.is_ok(),
        "Valid STREAM frame should parse successfully"
    );
...
fn test_connection_close_oversized_reason() {
    ...
    assert!(
        result.is_err(),
        "CONNECTION_CLOSE with oversized reason should fail"
    );
}
...
fn test_rapid_rtt_updates() {
    ...
    for i in 0..1000 {
        ...
        assert!(
            cubic.smoothed_rtt.is_some(),
            "smoothed_rtt should be initialized"
        );
        ...
        if i > 0 {
            assert!(
                cubic.min_rtt.unwrap() <= cubic.latest_rtt.unwrap(),
...
fn test_packet_loss_handling() {
    ...
    assert!(
        cubic.cwnd < 10000,
        "cwnd should be reduced after packet loss"
    );
    assert!(cubic.cwnd >= 2 * 1200, "cwnd should be at least 2 * MSS");
    assert_eq!(
        cubic.ssthresh,
        (10000.0 * 0.7) as u32,
        "ssthresh should be 0.7 * previous cwnd"
    );
}
