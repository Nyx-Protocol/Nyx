...
impl RekeyScheduler {
    ...
    pub async fn should_rekey(&self) -> (bool, RekeyTrigger) {
        ...
        if bytes >= self.config.bytes_threshold {
            debug!(
                "Rekey triggered by bytes: {} >= {}",
                bytes, self.config.bytes_threshold
            );
        ...
        if elapsed >= self.config.time_threshold {
            debug!(
                "Rekey triggered by time: {:?} >= {:?}",
                elapsed, self.config.time_threshold
            );
    ...
    pub async fn complete_rekey(&self, trigger: RekeyTrigger) {
        ...
        match trigger {
            RekeyTrigger::Bytes(bytes) => {
                ...
                info!(
                    "Rekey completed (bytes trigger): {} bytes transferred",
                    bytes
                );
            }
        ...
        if metrics.total_rekeys > 0 {
            metrics.avg_bytes_per_interval = metrics.total_bytes_transferred / metrics.total_rekeys;
        }
    ...
    pub async fn record_failure(&self) {
        ...
        warn!(
            "Rekey failure recorded (total failures: {})",
            metrics.rekey_failures
        );
    }
