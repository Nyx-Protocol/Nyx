...
mod test_s {
    ...
    async fn test_all_power_state_transitions() {
        ...
        let _ = nyx_mobile_ffi::nyx_power_set_state(NyxPowerState::Background as u32);
        let ev1 = timeout(Duration::from_millis(1000), rx.recv())
            .await
            .unwrap()
            .unwrap();
        assert!(
            ev1._detail.contains("\"state\":\"background\""),
            "Expected background, got: {}",
            ev1._detail
        );

        // Transition: Background → Inactive
        let _ = nyx_mobile_ffi::nyx_power_set_state(NyxPowerState::Inactive as u32);
        let ev2 = timeout(Duration::from_millis(1000), rx.recv())
            .await
            .unwrap()
            .unwrap();
        assert!(
            ev2._detail.contains("\"state\":\"inactive\""),
        ...
        let ev3 = timeout(Duration::from_millis(1000), rx.recv())
            .await
            .unwrap()
            .unwrap();
        assert!(
            ev3._detail.contains("\"state\":\"critical\""),
    ...
    async fn test_debounce_suppresses_rapid_toggles() {
        ...
        let _ = nyx_mobile_ffi::nyx_power_set_state(NyxPowerState::Background as u32);
        let ev1 = timeout(Duration::from_millis(1500), rx.recv())
            .await
            .unwrap()
            .unwrap();
        assert!(
        ...
        if let Ok(Ok(ev)) = result {
            ...
            assert!(
                ev._detail.contains("\"state\":\"inactive\"")
    ...
    async fn test_wake_counter_event() {
        ...
        assert_eq!(ev._ty, "power");
        assert!(
            ev._detail.contains("\"type\":\"Wake\""),
            "Expected Wake event, got: {}",
            ev._detail
        );
    }
    ...
    async fn test_resume_counter_event() {
        ...
        assert_eq!(ev._ty, "power");
        assert!(
            ev._detail.contains("\"type\":\"Resume\""),
            "Expected Resume event, got: {}",
            ev._detail
        );
    }
    ...
    async fn test_inactivity_trigger() {
        ...
        let ev = timeout(Duration::from_millis(2000), rx.recv())
            .await
            .unwrap()
            .unwrap();
        assert_eq!(ev._ty, "power");
    ...
    async fn test_activity_reset_on_active() {
        ...
        if let Ok(Ok(ev)) = result {
            assert!(
                !ev._detail.contains("\"type\":\"Inactivity\""),
                "Expected no Inactivity event after activity reset, got: {}",
                ev._detail
            );
        }
    }
...
mod unit_tests {
    ...
    fn test_map_power_to_screen_all_states() {
        assert_eq!(
            map_power_to_screen(NyxPowerState::Active as u32),
            ScreenState::On
        );
        assert_eq!(
            map_power_to_screen(NyxPowerState::Background as u32),
            ScreenState::Off
        );
        assert_eq!(
            map_power_to_screen(NyxPowerState::Inactive as u32),
            ScreenState::Off
        );
        assert_eq!(
            map_power_to_screen(NyxPowerState::Critical as u32),
            ScreenState::Off
        );
    ...
    fn test_display_power_all_states() {
        ...
        assert_eq!(
            display_power(NyxPowerState::Background as u32),
            "background"
        );
